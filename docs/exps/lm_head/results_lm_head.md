...
| **S1 predict-and-refine** | **24.48%** | **−7.01%** | **25.348** | **78.57** (+0.00) | **58.87** (+0.00) |
...
⚠️ **Novelty of S1 is not established.** The predict-and-refine *skeleton* is close to
SVD-softmax (Shim et al., NeurIPS 2017): low-rank preview → candidate set → exact
rescoring. What is new here is the **per-token adaptive prediction** and the activation-whitened
...
src/lm_head/
  ...
  screen_refine.py S1 — the per-token prediction + exact refine (Part 3)
...
| 0f | S1 per-token prediction beats a static one at matched reads | 0.00050 < 0.00125 |
...
| **per-token, from the prediction** | graded | **0.0003** | **100.00%** | **0.13%** |
...
case; a prediction only has to rank well on average, and a graded tail converts a prediction miss
...
It is the prediction, not the answer.

## 3c. S1 — predict-and-refine

```
stage 1  PREDICT   coarse logits for ALL V rows from the r0 largest coordinates of z
...
so predicting with a **projected hidden state** against the unrotated `W` is identical
...
| S1 *static prediction* | reads | 101.35% | 24.48% | −7.01% | 25.405 | 1.002 | 78.51 | −0.06 | 58.70 | −0.17 | .0011 | 100.000% |
...
perplexity 0.001 *lower* (noise), HellaSwag and ARC-C identical to two decimals. The prediction
did not miss the dense argmax once over the whole eval stream. At **12.65%** of reads the
cost is still ×1.003 / −0.05 pt.

**The prediction scoring is load-bearing — pure activation magnitude is not enough.** The
...
doing real work: the naive "big activations matter most" prediction would blow the C4 +1% clause,
where the full S1 prediction clears it by 3×. This is gate 0g's `|coef|`-only score, now measured
...
*pre-refine* prediction ranking, so it is a real block-accept rate, not 100% by construction —
...
| S1 `ceig` r0=192 N=8192, *static prediction* | 23.80% | 100.67% | 32.1182 | 1.0081 | 99.994% | 0.547% |
...
| S1 *static prediction* | reads | 100.67% | 23.80% | 47.21 | −0.11 | 34.22 | −0.26 | .0068 | 99.984% |
...
| S1 r0=384 N=8192, *static prediction* | 101.35% | 24.48% | −7.01% | 0.0011 | 100.00% | 0.155% | 0.00155 |
...
3. **Then it breaks.** 30B −4.69 at 5.56% and −24.40 at 2.41%; 0.6B −0.94 at 4.89% and −8.70
   ...
   ~85%, i.e. the prediction stops surfacing the right rows and the refine stage never sees them.
...
possible prediction, top-`r0` entries of `h` by `|hᵢ|` with no rotation and no column norm (gate
0g promoted to a sweep variant) — makes this concrete, and the two models disagree
instructively at the identical budget:

| | reads | KL | vs real prediction | C4 | HellaSwag | ARC-C |
|---|---|---|---|---|---|---|
...
On the 0.6B the crude prediction scores *above* dense while being 86× worse distributionally —
...
- **The crude prediction's KL is 22× apart across the models** (.6194 on the 0.6B, .0275 on the
  30B) at the same `D/16` prediction width. The standard basis is evidently far better aligned
...
|---|---|---|---|
| **all three: adaptive prediction + dynamic candidates + graded tail** | **.0017** | **×1.0017** | **+0.00** |
| prediction made static (a low-rank sketch) | .0068 | ×1.0081 | −0.11 |
...
- **The per-token adaptive prediction is the smallest of the three** (−0.11 pt, 3.8× in KL), and
...
2. **S1's novelty is unverified** — see the warning in the short version. The skeleton is
   likely SVD-softmax (2017); the adaptive prediction and the `ceig` rotation are the new parts,
...
| 6 | **F1** — reproduce/refute CSV-Decode's 18.4% \|S\|/V | **not run**; needs an external clone. Part 3 makes this less interesting than it was: S1 shows an *uncertified* prediction at ~25% of reads is free, so whether a *certified* one reaches 18.4% is now a question about certificates, not about the axis. |
| 7 | **Learned rotation** for the S1 prediction | **not run.** `ceig` is off-the-shelf; the basis that maximizes per-token coefficient concentration is an orthogonal-Procrustes problem, and §3b's energy table (95.21% at `r=256`) bounds what it could buy. Low priority at 24% of reads (KL .0003) — but §3e makes it interesting *below* 7%, where the prediction is what fails. |
...
| 9 | **Why the crude prediction's KL differs 22× across models** (§3e) | **not investigated.** `.6194` on the 0.6B vs `.0275` on the 30B at the same `D/16` width, i.e. the standard basis suits the 30B's head far better. Would predict how much the `raw` (zero-calibration) variant can be trusted on an unseen model. |
