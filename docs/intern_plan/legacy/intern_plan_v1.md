...
**The problem with LoRA:** $\Delta W = BA$ with $B \in \mathbb{R}^{d \times r}, A \in \mathbb{R}^{r \times d}$ caps the update at rank $r$. For post-compression recovery where the model needs to rearrange knowledge broadly, low rank is restrictive.

**FuRA's solution:** Perform a full-rank Block-TT (block-wise SVD) decomposition of the *pretrained weight itself*. For a weight $W \in \mathbb{R}^{m \times n}$, partition it into blocks and apply SVD per block: $W_i = U_i \Sigma_i V_i^\top$. Freeze all orthogonal bases $U_i, V_i$. Train only the diagonal factors $\Sigma_i$.

This gives:

- **Full-rank updates** — every singular direction in every block is adjustable (full rank, not rank-$r$)
- **Extreme parameter efficiency** — only $\sum_i \min(m_i, n_i)$ trainable params per matrix (vs. $2rd$ for LoRA)
- **Built-in importance ordering** — singular values are sorted within each block; $\sigma_1 \geq \sigma_2 \geq \cdots$; truncating at position $k$ gives the best rank-$k$ approximation per block

**Why it matters for elastic training:** FuRA's SVD decomposition provides exactly the nesting structure elastic models need. "Extract submodel at compression factor $c$" = "keep the top-$k$ singular components per layer." The importance ordering comes for free from the spectrum. This unifies adaptation and compression into one mechanism.
...
**Why this beats compress-then-train:**
...
**Budget loss:** $\mathcal{L}_{\text{router}} = \| \text{Cost}(c) - \hat{\mathcal{C}} \|$, where Cost can be parameter count or actual hardware latency. Star Elastic shows hardware-aware cost gives 5–10% better configs.

#### 3.2.1 detail version
...
### 3.3 Why Elastic Training Outperforms Select-then-Train
...
**Improvement 2: Compression-aware training (FuRA).**

During elastic training, decompose each weight as $W = U\Sigma V^\top$. Freeze $U, V$; train only $\Sigma$. The importance ordering of the diagonal is the elastic ordering — truncating $\Sigma$ at position $k$ gives the submodel. This means:

- No separate "importance ranking" step — the spectrum *is* the ranking
- Extraction is trivial: read off the first $k$ singular values
- FuRA's full-rank property means no capacity bottleneck during recovery
...
[1] Compute K_joint per layer/expert
    ...
    │
    ▼
[2] SVD decomposition per weight matrix (FuRA-style)
    W = U Σ V^T. The K_joint ranking aligns with
    the singular value ordering (reorder if needed).
    Freeze U, V.
    │
    ▼
[3] Elastic training loop:
    For each batch:
      • Sample target c.f. ~ Uniform[1.0, 2.0]
      • Per-layer budget allocation (learned allocator)
      • Extract submodel: keep top-k(layer) singular components
      • Forward + backward on submodel
      • L = L_LM + λ · L_budget
      • Update Σ diagonals + allocator parameters
    │
    ▼
[4] Extraction:
    Given target c.f., read optimal config from allocator,
    truncate Σ per layer, reconstruct W_compressed = U_k Σ_k V_k^T.
```
...
| 3    | Integrate training-aware compression (K_joint) + FuRA decomposition. Launch baseline runs (vanilla elastic vs. training-aware vs. FuRA). |
...
3. **Ablation: FuRA vs. full fine-tuning** during elastic training — quantify stability and efficiency gains.
...
**Training cost comparison** (approximate GPU-hours for a Qwen3-30B-A3B class model):

| Approach                            | Cost to get 3 model sizes                  |
| ----------------------------------- | ------------------------------------------ |
| Standard (train each independently) | ~3 × 50k GPU-hrs = 150k                   |
| Elastic (Stage 1)                   | ~50k + 10k FT = 60k                        |
| All-in-One (Stage 2)                | ~55k (single run with elastic regularizer) |

### 4.3 Core Idea

**Parameterization:** Decompose each weight as $W = U \Sigma V^\top$ and train *all three factors* ($U, \Sigma, V$) end-to-end.

**Why this produces elastic structure:** $\Sigma$ sits on the backpropagation path. Since $\sigma_i$ directly scales the $i$-th component's contribution to both the forward output and the backward gradient, SGD naturally routes more important information into higher-$\sigma$ components — the loss is more sensitive to them, so they attract larger updates. This is FuRA's key observation (Section 5): the spectral basis acts as an importance-aware preconditioner *during training*, not just at extraction time.
...
**Start with post-training (SFT/RL on a pretrained checkpoint).** Pretraining from scratch is expensive and risks conflating the elastic structure question with pre-training recipe issues. Post-training is:

- Cheap enough to iterate (days, not weeks)
- Directly relevant: the Macro pipeline already compresses pretrained models then post-trains
- Sufficient to demonstrate the core claim (spectral concentration during training)
...
1. **Post-training PoC (Weeks 5–8):** Take pretrained OLMoE-7B-1B, run SFT with FuRA ($U, \Sigma, V$ all trainable) + elastic regularizer. Extract submodels at various c.f. Compare against: (a) standard SFT + post-hoc compression, (b) Stage 1 elastic fine-tuning.
...
| ----- | ------------------------------------- | ----------------------------------------------------- | ----------- | ------------------------------------------------------- |
| 1     | Automatic compression config learning | Elastic training +$K_{\text{joint}}$ ranking + FuRA | Weeks 1–4  | Qwen3-30B-A3B at c.f.=1.5: <2pp drop (vs. 4pp baseline) |
